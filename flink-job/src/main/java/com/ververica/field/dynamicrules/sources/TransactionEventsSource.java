...
import com.ververica.field.dynamicrules.functions.TransactionEventsGenerator;
...
public class TransactionEventsSource {

  public static SourceFunction<String> createTransactionEventsSource(Config config) {
    ...
    switch (transactionsSourceType) {
      ...
      default:
        return new JsonGeneratorWrapper<>(new TransactionEventsGenerator(transactionsPerSecond));
    }
  }

  public static DataStream<TransactionEvent> stringsStreamToTransactionEvents(
      DataStream<String> transactionStrings) {
    return transactionStrings
        .flatMap(new JsonDeserializer<TransactionEvent>(TransactionEvent.class))
        .returns(TransactionEvent.class)
        .flatMap(new TimeStamper<TransactionEvent>())
        .returns(TransactionEvent.class)
        .name("TransactionEvents Deserialization");
  }
