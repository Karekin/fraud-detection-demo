...
public class BroadcastEmbeddedFlinkCluster<IN> implements Serializable {
    ...
    public void open(int dsSourcePort) throws Exception {
        ...
        SessionContext sessionContext = SessionContext.create(defaultContext, "default");
        sessionId = executor.openSession("default");
    ...
    public List<Tuple3<String, RowData, Long>> retrieveResults() throws InterruptedException {
        ...
        List<Tuple3<String, RowData, Long>> output = new ArrayList<>();
        for (RowData line : result)
            output.add(new Tuple3<>(sql, line, System.currentTimeMillis()));
        return output;
    }
    ...
    private List<RowData> cyclicRetrieveChangelogResult(
            LocalExecutor executor, String sessionId, String resultID, int iterations)
            throws InterruptedException {
        int iterCount = 0;
        final List<RowData> actualResults = new ArrayList<>();
        try {
