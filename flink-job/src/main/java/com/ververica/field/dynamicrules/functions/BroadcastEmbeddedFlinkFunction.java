...
public class BroadcastEmbeddedFlinkFunction<KEY, IN>
        extends KeyedBroadcastProcessFunction<KEY, IN, SqlEvent, Tuple3<String, RowData, Long>> {
    ...
    public void processElement(
            IN value, ReadOnlyContext ctx, Collector<Tuple3<String, RowData, Long>> out)
            throws Exception {
        try {
            ...
            for (BroadcastEmbeddedFlinkCluster<IN> cluster : clusters.values()) {
                List<Tuple3<String, RowData, Long>> output = cluster.retrieveResults();
                for (Tuple3<String, RowData, Long> line : output) {
                    out.collect(line);
                }
            }
        } catch (Exception e) {
    ...
    public void processBroadcastElement(
            SqlEvent value, Context ctx, Collector<Tuple3<String, RowData, Long>> out)
            throws Exception {
