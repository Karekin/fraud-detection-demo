...
public class ClassConditionSpec extends ConditionSpec {
    ...
    public IterativeCondition<?> toIterativeCondition(ClassLoader classLoader, Configuration globalConfiguration) throws Exception {
        ...
        Class<? extends IterativeCondition<?>> clazz = (Class<? extends IterativeCondition<?>>) classLoader.loadClass(this.getClassName());
        IterativeCondition<?> condition;
        if (AbstractCondition.class.isAssignableFrom(clazz)) {
            condition = clazz.getConstructor().newInstance();
            ((AbstractCondition<?>) condition).open(globalConfiguration);
        } else {
